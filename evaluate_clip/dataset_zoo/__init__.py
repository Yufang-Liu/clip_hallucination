from .aro_datasets import (VG_Relation, VG_Attribution, COCO_Order, Flickr30k_Order, Controlled_Images,
                           COCO_QA, VG_QA, COCO_Object, Flickr_Object, NoCaps_Object)
...
def get_dataset(dataset_name, image_preprocess=None, text_perturb_fn=None, image_perturb_fn=None, download=False, *args, **kwargs):
    ...
    if dataset_name == "VG_Relation": 
        ...
        return COCO_Object(image_preprocess=image_preprocess)
    elif dataset_name == 'Flickr_Object':
        return Flickr_Object(image_preprocess=image_preprocess)
    elif dataset_name == 'Nocaps_Object':
        return NoCaps_Object(image_preprocess=image_preprocess)
    else:
