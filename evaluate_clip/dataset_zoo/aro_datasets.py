...
from .constants import ARO_ROOT, COCO_ROOT, FLICKR_ROOT, NoCaps_ROOT
from .retrieval import pre_caption


class COCO_Object(Dataset):
    def __init__(self, image_preprocess=None, root_dir=COCO_ROOT, max_words=30, split="test",
        ...
        with open(json_file_path, 'r') as f:
            for line in f:
                ...
                label = data["label"]
                data["caption"][0], data["caption"][label] = data["caption"][label], data["caption"][0]
    ...
    def __getitem__(self, index):
        ...
        item = edict({"image_options": [image], "caption_options": test_case["caption"]})
        return item
    ...
    def evaluate_scores(self, scores):
        ...
        result_records = []
        result_records.append({"Accuracy": np.mean(correct_mask)})
        return result_records


class Flickr_Object(Dataset):
    def __init__(self, image_preprocess=None, root_dir=FLICKR_ROOT, max_words=30, split="test",
                 image_perturb_fn=None, download=False):
...
class NoCaps_Object(Dataset):
    def __init__(self, image_preprocess=None, root_dir=NoCaps_ROOT, max_words=30, split="test",
                 image_perturb_fn=None, download=False):
